...
fragment.querySelector('#day-top-section .contents-header').after(createHomeDayNav());
// 로맨스 TOP
const romanceTopWrapper = fragment.querySelector('#romance-top-section .contents-wrapper');
romanceTopData.forEach(data => {
    romanceTopWrapper.appendChild(createContentVerticalSmall(data));
});
fragment.querySelector('#recommend-event-section .contents-wrapper').appendChild(createEventSlide());
