...
#### <li> 적용되지 않던 .gitinore 파일을 재적용했습니다.

#### <li> 추상적인 이름었던 section의 이름을 webtoonSection으로 변경했습니다.

#### <li> json 파일을 형성했고, 이를 서버에서 라우팅으로 데이터를 응답받았습니다.

#### <li> 비동기 처리를 통해 필요한 데이터를 응답받은 다음 렌더링을 하도록 구현했습니다.

#### <li> 요일 클릭시 웹툰 콘텐츠를 바꾸는 함수를 응답받은 데이터를 기반으로 구현되도록 수정했습니다. 기존에 배열 데이터를 참조하던 다른 함수들도 바꿔줄 예정입니다.

#### <li> express generator로 생성되었던 불필요한 파일들을 삭제했습니다.
...
#### 1. 먼저 데이터를 응답받은 다음에 렌더링이 구현되도록 코드를 짰는데, 데이터가 필요하지 않는 렌더링 부분도 데이터를 응답받기 전까지 렌더링이 되지 않도록 구현했습니다. 지금처럼 데이터가 필요 하지 않는 렌더링 함수들도 데이터가 렌더링 뒤에 실행되도록 하는게 좋은 방법인지, 아니면 같은 랜더링이 목적인 함수이지만 데이터가 필요하지 않다면 먼저 렌더링 하는게 좋은 방법인지 궁금합니다.
