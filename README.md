...
### 1. 현제 함수에서 style 속성을 바꿀때, Tag.style.~~~ 을 통해서 바꾸고 있는데, 뭔가 css와 js가 혼용되 있다는 생각이 들었습니다. 현업에서는 어떠한 방법을 사용해서 함수에서 style 속성을 바꾸나요?

### 2. class는 css를 적용할 때 사용하고, 함수에서 `querySelector()`를 사용할 때는 가능하면 Id를 사용하라는 글을 본 기억이 있어 일단 그렇게 구현은 했는데, id로 테그를 선택하기 위해서 매번 id를 테그에 id를 추가하고 사용하고 있습니다. 이렇게 class와 id의 사용 용도를 분리하는 방법이 현업에서 많이 사용되는 방법일까요?
