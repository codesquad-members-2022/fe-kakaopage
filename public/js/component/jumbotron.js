const carouselBtn = (data) => {
  const template = `
    <button class="toon-jumbotron_left"  data-action='carousel' "><img src=${data.icon.leftBtn}></button>
    <button class="toon-jumbotron_right" data-action='carousel' "><img src=${data.icon.rightBtn}></button>`;
  return template;
};
...
export const jumbotron = (data) => {
  ...
  return `
    <div class="toon-jumbotron_slides">
      ${template}
    </div>
    ${carouselBtn(data)}`;
};
